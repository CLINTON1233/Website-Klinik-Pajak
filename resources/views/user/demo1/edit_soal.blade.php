        ...
        <div class="container">
            <div class="page-inner">
                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        <form action="{{ route('update_soal', ['id' => $soal->id]) }}" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="pertanyaan" class="form-label">Pertanyaan</label>
                                <textarea class="form-control" id="pertanyaan" name="pertanyaan" rows="3" placeholder="Pertanyaan Soal">{{ $soal->soal }}</textarea>
                            </div>
                            <div class="mb-3">
                                <label for="skor" class="form-label">Skor</label>
                                <input type="number" class="form-control" id="skor" name="skor" placeholder="Skor Soal" value="{{ $soal->skor }}">
                            </div>
                            <div class="mb-5">
                                <p>Tipe Soal</p>
                            ...
                            <div id="soalPilihanGanda" class="{{ $soal->tipe_soal == 'Pilihan Ganda' ? '' : 'visually-hidden' }}">
                                <div class="mb-3">
                                ...
                                <div class="row gap-3">
                                    ...
                                    <div class="d-flex grid gap-1">
                                        ...
                                        <input type="text" class="form-control" id="pilihan_{{ strtolower($option) }}" name="pilihan_{{ strtolower($option) }}" placeholder="Isi Pilihan Ganda" value="{{ $soal['pilihan_' . strtolower($option)] }}">
                                        <div class="form-check form-check-inline">
                                            <input type="radio" class="form-check-input" name="jawaban" id="jawaban_{{ strtolower($option) }}" value="{{ strtolower($option) }}" {{ $soal->jawaban == strtolower($option) ? 'checked' : '' }}>
                                        </div>
                                    </div>
                                    @endforeach
                                </div>
                            </div>
                            <div id="soalEsai" class="{{ $soal->tipe_soal == 'Esai' ? '' : 'visually-hidden' }}"></div>
                            <div class="mt-5" id="submit">
                                <button type="submit" class="btn text-white">Simpan Perubahan</button>
                            </div>
                        </form>
                    </div>
                </div>
